...
\begin{document}
	...
	\begin{align}
		...
		\label{eq:dist_set}
	\end{align}
	where $a,\, b \in 2\mathbb{Z}^{+}$. The disturbance set is assumed to \textcolor{blue}{be bounded by appropriate values and} contain the origin. Furthermore, it is assumed that the rear wheels cannot be steered. Therefore, the control input to the model \eqref{eq:mod}, represented by $\gamma\left(t\right) = \left(a\left(t\right),\, \delta_f\left(t\right)\right)$, is the acceleration and front steering angle pair. 
 ...
 \textcolor{blue}{Intuitively, we impose higher weights on first three vectors to avoid catastrophes, otherwise the weights are relaxed as,}
 \begin{equation}\label{weights_order}
    \color{blue}
     \phi_1 > \phi_2 > \phi_3 \gg \phi_4 > \phi_5 > \phi_6. \nonumber
 \end{equation}
\textcolor{blue}{and these are tuning parameters that vary depending on prioritized tasks.}
