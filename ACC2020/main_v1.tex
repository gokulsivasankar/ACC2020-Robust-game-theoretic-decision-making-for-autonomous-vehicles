...
\begin{document}
	...
	\end{itemize}
	 ...
	 In a multi-agent traffic scenario, the interactive nature of the decision making process is captured by the features defined in the previous subsection. 
	 
	 features of the reward function take into account the states of the other agents in the 
	 
	 
	 
	 Based on the definitions of the features, the rewards of a vehicle not only depend on its own states and actions, but also depend on the states and actions of its opponent vehicle . Such an interdependence reflects the interactive nature of vehicle decision making in a multi- vehicle traffic scenario. 


	 
	 To compute the cumulative reward in \eqref{eq:cum_reward}, the state of the traffic, $s_{t+j}$, at prediction steps $j=0,\,1,\,\cdots,\,N-1$, are predicted using level-k game theoretic model.
...
<<<<<<< HEAD
	We would like to express out appreciation to Nan Li who provided expertise that greatly assisted the research and it should be noted that the code publicly available (\url{https://github.com/gokulsivasankar/RobustDecisionMaking}) is based on his previous study \cite{Li2018}.
=======
	We would like to express out appreciation to the Anouck Girard and Nan Li who provided expertise that greatly assisted the research and it should be noted that the code publicly available (\url{https://github.com/gokulsivasankar/RobustDecisionMaking}) is based on his previous study \cite{Li2018}.
>>>>>>> 72e8a2cf5dfa8b3b2c70d18e369d3c9305d13e17
