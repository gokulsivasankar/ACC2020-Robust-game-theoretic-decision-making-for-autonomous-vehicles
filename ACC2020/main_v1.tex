...
\begin{document}
	...
	\subsection{Notation}
	The symbol $\mathbb{Z}_{\left[a,\, b\right]}$ denotes a set of consecutive integers from $a$ to $b$ and $2\mathbb{Z}^{+}$ denotes set of positive even integers. The $L_1$ norm of a vector $x$ is denoted by $|x|$. The operator $\ominus$ denotes the Pontryagin difference, defined for the sets $\mathcal{A}$ and $\mathcal{B}$ as
	\begin{equation}
		\mathcal{A}\ominus\mathcal{B}\coloneqq\left\{ a|a+b\in\mathcal{A}\,\forall b\in\mathcal{B}\right\}.\label{eq:pontdiff}
	\end{equation}
	
	$0_{m\times n}$ represents a zero matrix of size $m\times n$, $I_n$ denotes an $n\times n$ identity matrix and $\boldsymbol{1}_n$ is a $n \times 1$ vector of ones. The operator $\det\left(A\right)$ denotes the determinant of the matrix $A$. $A \succ 0$ represents a positive definite matrix $A$. ; ; and $\|A\|_{\max}\coloneqq\underset{i,j}{\max}\,\left|a_{ij}\right|$, where $\left|a_{ij}\right|$ is the absolute value of the element in $i^{th}$ row and $j^{th}$ column of the matrix $A$. 
