...
\begin{document}
	...
	The mismatch between the actual position of the center of mass of the vehicle which is used to determine the rectangular outer approximation of the vehicle (see Section \ref{sec:reward_function}) and its predictions obtained using the dynamic model in \eqref{eq:nom_mod} might lead to collision. In the multi-agent traffic scenario under consideration, there are two sources of modeling errors: (i) the uncertainties, $w^{(m)} = \left(w_x^{(m)},\,w_y^{(m)}\right) \in \mathcal{W}_m$, resulting due to the use of a simplified model in \eqref{eq:nom_mod}; and (ii) the uncertainty, $w^{(d)} = \left(w_x^{(d)},\,w_y^{(d)}\right) \in \mathcal{W}_d$, arising due to improper estimation of the driver model. Hence, the disturbance set defined in \eqref{eq:dist_set} is $\mathcal{W} = \mathcal{W}_m \oplus \mathcal{W}_d$. 
	
	
	Robust approaches are required that can account for these uncertainties while computing the control actions. 


	\begin{align}
		\bar{\mathcal{W}}_{i}^{l} = \mathcal{W}_m \oplus P_{K_{i}^l = 0} \mathcal{W}_d
	\end{align}

	The safe zone of an agent $i$ from the perspective of the ego vehicle $l$, $s_{i}^{l}$, is defined as
	\begin{align}
		s_{i}^{l} = c_{i} \oplus \bar{\mathcal{W}}_{i}^{l},	
	\end{align}

	\noindent where $c_{i}$ is the collision avoidance zone of $i$.

	If the driver is level-0 (aggressive), we 
	
	
	
	Feedback min-max \cite{Scokaert1998} strategy can be used
	
	Level-1 drivers are cautious and 




	The optimal control action chosen by an autonomous agent $l$, $\optact{l}$, which is the first element of the control sequence, $\optactseq{l}$, is chosen such that the highest cumulative reward is expected by solving
	\begin{subequations}
		\begin{align}
			\boldsymbol{\gamma^{*}_{t}[l]} =  & \arg \underset {\boldsymbol{\gamma_{t}[l]} \in  \boldsymbol{\Gamma}} {\max}\, \, \underset{w_{t+j}^{p} \in \mathcal{W}'_i}{\min} \, \, \mathcal{R}_P\left(\boldsymbol{\gamma_{t}[l]}\right) \\
			\textrm{s. t.} \forall &  j \in \mathbb{Z}_{0,\,N-1},\, \eqref{eq:psi_pred},\, \eqref{eq:v_pred},\, \eqref{eq:beta_pred} \nonumber \\
	...
	\noindent where $w_{t+j}^{p} = \left(w_{x,\,t+j}^{p},\, w_{y,\,t+j}^{p}\right)$
